...
- 拖动小狗：把它放在不遮挡工作的屏幕角落。
- 召唤宠物：在设置页的“宠物互动”、托盘菜单或宠物右键菜单中选择“召唤宠物”，把它带到鼠标附近；已隐藏的宠物也会重新出现。
- 跟随鼠标：在相同入口开启“跟随鼠标”，小狗会跟着鼠标跑，靠近后停下。默认关闭，开关偏好会自动保存；关闭后停留在当前位置。
- 右键小狗或托盘图标：打开设置、切换显示状态或退出。

召唤和跟随均支持 macOS 与 Windows，共用 Electron 的 `screen.getCursorScreenPoint()`；召唤使用 `BrowserWindow.setBounds()`，连续跟随仅用 `setPosition()` 移动窗口，无需辅助功能权限或额外原生依赖。坐标统一使用逻辑像素（DIP），适配 Retina、高 DPI、多显示器及负坐标屏幕；跨屏时会进入鼠标所在屏幕的可用区域，避免卡在屏幕间隙或任务栏/Dock 外。

跟随采用 65ms 指数滤波与临界阻尼平滑，保留浮点坐标并限制速度。目标偏离超过 22 DIP 才起跑；靠近到 6 DIP 内且低速持续 180ms 才停止，避免轻微手抖反复切换跑动/静止。转向和跨屏需稳定 120ms，屏幕边缘的左右侧选择也有滞回，避免来回跳变。参数集中在 `src/main/petMovement.ts`。

拖拽、打开菜单、显示气泡提醒和专注期间会暂停跟随，结束后自动继续；休息奔跑优先于跟随。隐藏宠物不会被跟随自动唤回，必须手动显示或召唤。移动时复用当前宠物的休息奔跑动画（自定义宠物按原有规则回退），停止后恢复原来的作息状态。每种状态缓存其动画变体，启停不会重新随机换图；新 GIF 加载完成前保留旧图，避免空白闪动，长期状态仍保留每 5 分钟轮换。
