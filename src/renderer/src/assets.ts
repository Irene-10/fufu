...
export function getPetAsset(
  ...
  if (asset.isPlaceholder && !warnedPlaceholders.has(warningKey)) {
    ...
    console.warn(`Fufu is using a placeholder asset for ${warningKey}.`);
  }

  const src = new URL(window.fufu.assetUrl(selectedPath));
  if (replayKey > 0) {
    src.searchParams.set("fufuReplay", String(replayKey));
  }
